...
sample.data%>%
  group_by(EH_ID)%>%
  select(EH_ID, weight_dpi0)%>%
  distinct()%>%
  left_join(exp.des, by= "EH_ID")%>%
  select(EH_ID, weight_dpi0, ageAtdpi0expe1a, Sex, Locality, Genome, Strain)%>%
  rename(Weight_at_DPI_0= weight_dpi0, Age_at_DPI_0 = ageAtdpi0expe1a)-> tmp1
